...
const {isUser} = require ('./middleware/middle.js')
...
app.post('/projet/arbres', isUser, (req, res) => {
    ...
    fiche.save()
      ...
      .catch(() => res.status(400).json({ message: 'Erreur de transmission' }));//noralemenr error à la place de ()//
  });
...
app.get("/projet/:id", async (request, response) => {
  const fiche = await ficheModel.find({_id: request.params.id});
...
app.delete('/projet/:id', isUser, (req, res) => {
    ficheModel.deleteOne({_id: req.params.id})
...
app.put('/projet/:id', isUser, (req, res) => {
    ...
    ficheModel.updateOne({ _id: req.params.id }, { ...req.body, _id: req.params.id })
            ...
            .catch(() => res.status(400).json({ message: 'Echec de la modification' }));//noralemenr error à la place de ()//
        }
    );
